...
run:	build	
	...
	sudo docker exec $(CONTAINER_NAME) sed -i 's/^tls-priorities =.*/tls-priorities = "NORMAL:-VERS-ALL:+VERS-TLS1.3:+VERS-TLS1.2:-CIPHER-ALL:+AES-128-GCM:+AES-256-GCM:+CHACHA20-POLY1305:-GROUP-ALL:+GROUP-X25519:+GROUP-SECP256R1:%SERVER_PRECEDENCE:%COMPAT"/' /etc/ocserv/ocserv.conf
